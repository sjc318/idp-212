...
leftPath = ("rc", "rc", "rc", "rc", "rc", "rc", "r", "l", "rc", "rc", "rc", "rc", "rc", "rc")
...
leftBoxesUnloaded = 2
rightBoxesUnloaded = 2

robotStatus = "returnToNode"
currentBoxColour = "blue"
...
def bayPathCalculation(colour, onLeft):
    ...
    if onSecondFloor==True:
        if(colour == "red"):
            path += ["rr","l","s","s","s","s","s","s","s"] #reverse right
        elif(colour == "blue"):
            path += ["rr","r","s","s","s","s","s","s","s"]
        elif(colour == "yellow"):
            path += ["rr","l", "s", "s", "s", "s", "s", "s", "r", "l"]
        elif(colour == "green"):
            path += ["rr","r","s","s","s","s","s","s","l","r"]
        return path
    elif(onLeft):
        if(colour == "red"):
            path += ["l", "s", "s", "s", "r"]
        ...
        if updatedBranchProfile == [False, True, True]:
            ...
            if path[pathIndex] == "r":
                ...
                if(checkForBox(False, tofs) < 300):
                    ...
                    print("Going straight")
                    decisionSkipTime = straightTime
            elif path[pathindex]=="rr":
                motors[0].Forward(60)
                motors[1].Forward(60)      #moving motors forward
                time.sleep(1)
                reverseRight(sensors,motors)
                while branchProfile != [True,True,True]:
                    readings=read_sensors(sensors)
